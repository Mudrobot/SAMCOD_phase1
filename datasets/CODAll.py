...
class CODAllDataset(Dataset):
    def __init__(self, cfg, image_root, gt_root, transform=None, split=False, training=False, if_self_training=False, eval_dataset=None):
        self.cfg = cfg
        # self.root_dir = image_root
        self.transform = transform
        if training:
            train_all_images = [os.path.join(image_root, 'TrainDataset/Imgs', f) for f in os.listdir(os.path.join(image_root, 'TrainDataset/Imgs')) if f.endswith('.jpg')]
            train_all_images = sorted(train_all_images)
            self.root_dir = os.path.join(image_root, 'TrainDataset/Imgs')
        else:
            if eval_dataset == 'cod':
                eval_images = [os.path.join(image_root, 'TestDataset/COD10K/Imgs', f) for f in os.listdir(os.path.join(image_root, 'TestDataset/COD10K/Imgs')) if f.endswith('.jpg')]
                eval_images = sorted(eval_images)
                self.root_dir = os.path.join(image_root, 'TestDataset/COD10K/Imgs')
            elif eval_dataset == 'camo':
                eval_images = [os.path.join(image_root, 'TestDataset/CAMO/Imgs', f) for f in os.listdir(os.path.join(image_root, 'TestDataset/CAMO/Imgs')) if f.endswith('.jpg')]
                eval_images = sorted(eval_images)
                self.root_dir = os.path.join(image_root, 'TestDataset/CAMO/Imgs')
            elif eval_dataset == 'cham':
                eval_images = [os.path.join(image_root, 'TestDataset/CHAMELEON/Imgs', f) for f in os.listdir(os.path.join(image_root, 'TestDataset/CHAMELEON/Imgs')) if f.endswith('.jpg')]
                eval_images = sorted(eval_images)
                self.root_dir = os.path.join(image_root, 'TestDataset/CHAMELEON/Imgs')
            else:
                raise NotImplementedError("Not implemented")
        
        if training:
            random.shuffle(train_all_images)
            images = train_all_images
        else:
            random.shuffle(eval_images)
            images = eval_images

        self.images = images
        self.gts = [os.path.join(self.root_dir.replace('Imgs','GT'), os.path.basename(image_path.replace(".jpg", ".png"))) for image_path in self.images]
        self.filter_files()
    ...
    def filter_files(self):
        ...
        for img_path, gt_path in zip(self.images, self.gts):
            ...
            if img.size == gt.size and not np.all(np.array(gt) == 0) and not np.all(np.array(gt) == 255):
    ...
    def __getitem__(self, idx):
        ...
        if self.if_self_training:
            ...
            return image_weak, image_strong, torch.tensor(bboxes_weak), torch.tensor(masks_weak).float(), os.path.basename(self.images[idx])
            ...
            bboxes = np.stack(bboxes, axis=0)
            masks = np.stack(masks, axis=0)
            return image, torch.tensor(bboxes), torch.tensor(masks).float(), os.path.basename(self.images[idx])


class CODAllDatasetwithCoarse(CODAllDataset):
...
def load_datasets(cfg, img_size):
    ...
    train_each_dataloader = DataLoader(
        train_each,
        batch_size=cfg.batch_size,
        shuffle=False,
        num_workers=cfg.num_workers,
    ...
    return train_dataloader, val_dataloader, train_each_dataloader
...
if __name__ == "__main__":
    from tools import ResizeAndPad, soft_transform, collate_fn, collate_fn_, decode_mask
